    ...
    def build(self, hp):
        ...
        for i in range(hp.Int('num_layers', min_value=0, max_value=2, step=1)):
            x = GRU(units=hp.Choice('units', values=[32, 64, 128, 256]),
                    dropout=hp.Float('dropout', min_value=0.0, max_value=0.1),
                    recurrent_dropout=hp.Float('recurrent_dropout', min_value=0.0, max_value=0.1),
                    return_sequences=True)(x)

        x = GRU(units=hp.Choice('units', values=[32, 64, 128, 256]),
                    dropout=hp.Float('dropout', min_value=0.0, max_value=0.1),
                    recurrent_dropout=hp.Float('recurrent_dropout', min_value=0.0, max_value=0.1))(x)
